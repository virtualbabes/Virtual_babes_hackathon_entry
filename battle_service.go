		...
		if n.boundaryCheck(gridIndex) && match.Board[nbIdx] != nil {
			...
			if neighbor.Owner != pIdx && pPower > nPower {
				...
				if match.Rules["Fallen_penalty"] {
					...
					// PILLAR 3: Immediate Scars Persistence.
					// Ensures degradation survives Sudden Death or DNF.
					l.inventory[neighbor.ID] = *neighbor
					l.persistentCardCache[neighbor.ID] = *neighbor
				}
			...
			for _, idx := range indices {
				if match.Board[idx].Owner != pIdx {
					...
					if match.Rules["Fallen_penalty"] {
						...
						l.inventory[match.Board[idx].ID] = *match.Board[idx]
						l.persistentCardCache[match.Board[idx].ID] = *match.Board[idx]
					}
...
func (l *Lobby) finalizeMatchResultLocked(winnerID string, deck []int, history MatchHistory) {
	...
	if wallet, ok := l.wallets[winnerID]; ok {
		...
		// PILLAR 3: Atomic Finalization.
		// Process Rating and Achievements while the lock is held to prevent race conditions.
