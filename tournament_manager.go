...
func (l *Lobby) handleTournamentRegister(w http.ResponseWriter, r *http.Request) {
	...
	l.mutex.Lock()
	// PILLAR 3: Double-Checked Locking. Ensure state didn't change while waiting for the write lock.
	if _, isProcessing := l.processingRegistrations[targetWallet]; isProcessing {
		l.mutex.Unlock()
		http.Error(w, "Registration already in progress for this wallet", http.StatusConflict)
		return
		...
		for _, tx := range res.Transfers {
			if strings.HasPrefix(tx.Metadata, "VBT_TOURN_SUMM:") {
				...
				if err := json.Unmarshal([]byte(strings.TrimPrefix(tx.Metadata, "VBT_TOURN_PAYOUT:")), &data); err == nil && data.TID != "" {
					...
					// Deduplicate: multiple transfers in same group have same Group ID
					alreadyAdded := false
					for _, id := range payoutTxIDs[data.TID] {
						if id == targetID {
							alreadyAdded = true
							break
						}
					...
					if !alreadyAdded {
						payoutTxIDs[data.TID] = append(payoutTxIDs[data.TID], targetID)
					}
				}
			}
		}
...
func (l *Lobby) processTournamentResult(matchID, winnerWallet string) {
	...
	// Iterate by index to ensure the analyzer recognizes reads from the master slice.
	for i := range l.tournament.Matches {
		m := &l.tournament.Matches[i]
...
func (l *Lobby) advanceTournamentRound() {
	...
	for i := 0; i < len(roundWinners); i += 2 {
		...
		if i+1 >= len(roundWinners) {
			...
			log.Printf("[TOURNAMENT] Generated BYE match: %s", l.tournament.Matches[len(l.tournament.Matches)-1].ID)
			continue
		}
		l.tournament.Matches = append(l.tournament.Matches, TournamentMatch{
...
func (l *Lobby) determineTop5(matches []TournamentMatch, winner string) []string {
	...
	// Helper to identify Regional Governors (2+ territories) with case-insensitive verification.
	isGov := func(wallet string) bool {
		w := strings.ToLower(wallet)
		for _, club := range l.clubs {
			if strings.EqualFold(club.OwnerWallet, w) && len(club.Territories) >= 2 {
				return true
			}
		}
		return false
	...
	// PILLAR 3: Identity Safety.
	// Nil-safe reputation lookup to prevent sorting panics.
	getRep := func(wallet string) int {
		if stats, ok := l.leaderboard[strings.ToLower(wallet)]; ok {
	...
	// PILLAR 1: Tiered Tie-breaker. Prioritize Regional Governors, then sanitized Reputation.
	sort.Slice(semiLosers, func(i, j int) bool {
		govI, govJ := isGov(semiLosers[i]), isGov(semiLosers[j])
		if govI != govJ {
			return govI // Prioritize Governor
		}
		return getRep(semiLosers[i]) > getRep(semiLosers[j])
	})
	...
	if len(top5) < 5 {
		...
		sort.Slice(quartLosers, func(i, j int) bool {
			...
			return getRep(quartLosers[i]) > getRep(quartLosers[j])
		})
...
func (l *Lobby) finalizeTournament(winners []string) {
		...
		for i, player := range top5 {
			...
			go func(p string, rank int, amt uint64) {
				...
				if err != nil {
					...
					// PILLAR 4: Forensic Encoding Parity.
					// Use Base64 to match Indexer GroupID strings for verification parity.
					gidB64 := base64.StdEncoding.EncodeToString(gid[:])
					mu.Lock()
					payoutTxIDs = append(payoutTxIDs, gidB64)
					mu.Unlock()
					log.Printf("[TOURNAMENT] Payout successful for rank %d (%s). GID: %s. Skipped: %v\n", rank+1, p, gidB64, strings.Join(skipped, ", "))
					l.broadcastToAdmins(fmt.Sprintf("🏆 <b>TOURNAMENT PAYOUT:</b> Rank %d (%s) received rewards. GID: %s. (Skipped: %v)", rank+1, p, gidB64, strings.Join(skipped, ", ")))
				}
			}(player, i, shareMicro)
		}
		wg.Wait()
	...
	if len(payoutTxIDs) > 0 {
		// PILLAR 4: Financial Proof.
		// Deduplicate Group IDs before hashing to ensure consistency with the
		// reconstruction logic in handleTournamentHistory.
		uniqueIDs := make(map[string]bool)
		var deduped []string
		for _, id := range payoutTxIDs {
			if !uniqueIDs[id] {
				uniqueIDs[id] = true
		...
		sort.Strings(deduped) // Ensure determinism
		hashInput := strings.Join(deduped, ",")
		h := sha256.Sum256([]byte(hashInput)) // Hash of GIDs
		payoutsHash = hex.EncodeToString(h[:])
		log.Printf("[TOURNAMENT] Payouts Hash generated: %s (from %d unique GIDs)\n", payoutsHash, len(deduped))
	}
...
func (l *Lobby) dispatchTournamentRewards(recipient string, rank int, potShareMicro uint64) (types.Digest, []string, error) {
	...
	// Track the portion of the payout attributed to the reserved tournament pot
	potPortionMicro := uint64(float64(potShareMicro) * multiplier)
	appliedPotPortionMicro := uint64(0)
	...
	for appIDStr, baseAmt := range activeRewards {
		...
		// Only assign appliedPotPortionMicro if the primary reward asset is actually going to be dispatched.
		// This prevents incorrect deduction from pendingTournamentPayouts if the asset is skipped.
		if appIDStr == rewardAsset {
			appliedPotPortionMicro = potPortionMicro
		}
	...
	for i := range txns {
		txns[i].Group = gid
		_, stxn, _ := crypto.SignTransaction(faucetAccount.PrivateKey, txns[i])
