...
* **Blockchain Integration:** Primary support for Voi (ARC-200 tokens/NFTs) and Algorand. Uses indexers for metadata, verification, and **blockchain-native state reconstruction** (leaderboards, economy, card cache).
* **Security Model:** "Switchboard Pattern" (server-side signing for payouts; client-side nonce proofs to prevent replay attacks). Zero client-side private key exposure. Authoritative state is exclusively reconstructed from **GZIP-compressed blockchain snapshots**.
...
* `lobby_manager.go`: Real-time state manager and orchestrator of the **Snapshot Pattern**. Manages the compression and dispatch of `VBT_STATE_SNAPSHOT` and `VBT_ECONOMY_SNAPSHOT` notes to the ledger.
...
* `economy_service.go`: Manages global economic health and dynamic reward scaling. Seasonal metadata (initial rewards, season start) is now persisted directly to the blockchain economy snapshot.
...
* `handlers_criminality.go`: Endpoint management for kidnapping gambits and heist results. Leverages the blockchain to restore hostage states after restarts.
...
* `backend_types.go`: Server-only types and high-frequency logic (Mojo decay) isolated via build tags.
...
3. **Clubs & Territories:** Guild system where clubs control districts, earning revenue from shop turnover and providing member buffs (+5% power). **Includes Mojo Surge achievement and District Stabilizer buff.**
...
| **Persistence** | **HARDENED** | Event Sourcing Archival | Database-less Snapshot Pattern is live. |
