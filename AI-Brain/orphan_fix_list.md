...
- [FIXED] Ensured `use_item` handler correctly deducts from inventory and applies effects to `PlayerStats` or `MatchState`.
...
- [FIXED] Consolidated duplicated administrative authentication logic in `handlers_admin.go` and verified multi-chain signature support.
...
- [FIXED] Resolved metadata fragmentation by implementing a unified `MetadataDispatcher` in `oracle_service.go`, enabling native support for ARC-19 and ARC-69 assets alongside ARC-72.
- [FIXED] Corrected property inheritance in `_cards.scss` for power values by targeting generic `div` elements generated by `renderCardHTML` template.
- [FIXED] Optimized 3D territory map for mobile viewports by flattening rotation and forcing a strict 3-column grid layout in `_territory.scss`.
- [FIXED] Hardened `CheckOrigin` in `server.go` to implement strict domain filtering via `ALLOWED_ORIGINS`, securing the WebSocket switchboard against unauthorized third-party connections.
...
- [VERIFIED] `handleHeist` (club_service.go) correctly influences 'Kidnap Gambit' success chance based on the target club's 'Security' staff count.
