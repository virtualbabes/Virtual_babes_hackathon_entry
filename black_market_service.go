...
func (l *Lobby) handleSellMarketTokens(w http.ResponseWriter, r *http.Request) {
	...
	l.logAdminAuditLocked("TOKEN_LIQUIDATION", wallet, fmt.Sprintf("Sold %d tokens for %.2f $VBV", req.Amount, float64(vbvGainMicro)/1000000.0))
...
func (l *Lobby) handleBuyBlackMarket(w http.ResponseWriter, r *http.Request) {
	...
	saleDetails := map[string]interface{}{
		...
		"sector_id":      loan.TerritoryID, // PILLAR 3: Localized Economic Auditing.
		"ts":             time.Now().Unix(),
	}
	...
	l.logAdminAuditLocked("BLACK_MARKET_BUY", wallet, fmt.Sprintf("Scavenged %s for %.2f $VBV", loan.ID, float64(scavengePrice)/1000000.0))
	msg := l.getLobbyUpdateMsgLocked()
	go func() { l.broadcast <- msg }()
	w.Header().Set("Content-Type", "application/json")
